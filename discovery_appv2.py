...
CATEGORY_KEYWORDS = {
    "t-shirt": ["t-shirt", "tshirts", "tshirt", "tee", "tees", "shirt", "shirts"],
...
def fetch_duckduckgo_products(query: str, brand: str = None) -> list:
    """Returns DuckDuckGo text search results as-is."""
    ddg_products = []
    search_term = f"buy {brand} {query}" if brand else f"buy {query}"
    try:
        with DDGS() as ddgs:
            ...
            for result in text_results:
                ...
                # Extract domain name for brand display
                domain = url.split("/")[2].replace("www.", "") if "://" in url else "Web Search"
                ...
                ddg_products.append(
                    {
                        ...
                        "description": snippet,
                        "url": url,
                        "image_url": "https://via.placeholder.com/200x200?text=Web+Result",
                    }
                )
    except Exception:
        pass
...
def search_shopify_dynamic(prompt: str = "") -> list:
    """Smart router: If external brand is requested, routes to DDG. Deduplicates results by URL."""
    ...
    if is_external_brand:
        return fetch_duckduckgo_products(query=prompt, brand=target_brand)
    ...
    # Deduplicate by URL instead of brand so all valid results pass through
    unique_products, seen_urls = [], set()
