...
# Load Embedding Model and Tokenizer (cached to prevent reload on every run)
@st.cache_resource
...
def compute_embeddings(text_list: list) -> torch.Tensor:
    """Computes mean-pooled normalized embeddings for a list of text strings."""
...
def filter_by_cosine_similarity(
    ...
    # Build textual representations for candidates
    product_texts = [
        f"{p['title']}: {p['description']}" for p in products
    ]

    # Compute dense embeddings for query and product candidates
    query_embedding = compute_embeddings([query])  # Shape: [1, dim]
    product_embeddings = compute_embeddings(product_texts)  # Shape: [N, dim]

    # Compute Cosine Similarity Matrix (dot product of normalized vectors)
    cosine_similarities = (
        torch.mm(query_embedding, product_embeddings.transpose(0, 1))
        .squeeze(0)
        .tolist()
    )

    # Attach score & filter based on threshold
    filtered_products = []
    for prod, score in zip(products, cosine_similarities):
        prod["similarity_score"] = round(float(score), 4)
        if score >= threshold:
            filtered_products.append(prod)

    # Sort candidates by highest visual/semantic similarity
...
def search_shopify_dynamic(prompt: str = "") -> list:
    """Parallel multi-store search executor with Cosine Similarity filtering."""
    ...
    # --- COSINE SIMILARITY FILTERING (Threshold >= 0.85) ---
    all_matched_products = filter_by_cosine_similarity(
        query=prompt, products=all_matched_products, threshold=0.85
    )
...
def display_carousel_html(products: list) -> str:
    """Generates the HTML carousel string including similarity score tag."""
    if not products:
    ...
    for item in products:
        ...
        score_display = (
            f"<span style='font-size: 10px; background: #e0f7fa; color: #006064; padding: 2px 6px; border-radius: 4px; font-weight: bold;'>Match: {int(item.get('similarity_score', 0) * 100)}%</span>"
